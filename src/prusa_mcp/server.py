...
_SCHEMA: dict[str, dict] = json.loads(_SCHEMA_PATH.read_text()) if _SCHEMA_PATH.exists() else {}
...
def load_project(path: str) -> str:
    ...
    summary = {k: _annotate(k, _config[k]) for k in _SUMMARY_KEYS if k in _config}
...
def get_config(keys: list[str] | None = None) -> str:
    ...
    """
    _require_project()
    items = _config if keys is None else {k: _config[k] for k in keys if k in _config}
...
def set_config(settings: dict[str, str]) -> str:
    ...
    project.write_config(_project_path, _config)  # type: ignore[arg-type]
...
async def slice_model(output_path: str | None = None) -> str:
    ...
    """
    _require_project()
    out = Path(output_path) if output_path else _project_path.with_suffix(".gcode")  # type: ignore[union-attr]
    stats = await slicer.slice_project(
        _project_path,  # type: ignore[arg-type]
        out,
        cli_path=_cli_path(),
        datadir=_datadir(),
    )
...
def open_in_gui() -> str:
    ...
    _require_project()
    subprocess.Popen(["xdg-open", str(_project_path)])
    return f"Opened {_project_path.name} in GUI"  # type: ignore[union-attr]
...
def _annotate(key: str, value: str) -> dict:
    ...
    for field in ("label", "sidetext", "tooltip", "min", "max", "help_url"):
        if field in info:
    ...
    if "enum_values" in info:
        ...
        entry["valid_values"] = [
            ...
            for v, lbl in zip(info["enum_values"], labels)
        ]
...
def _validate(key: str, value: str) -> str | None:
    ...
    if value == "nil" and not info.get("nullable"):
        return f"not nullable — use a concrete value or omit"
